...
function SOTA_HandlePlayerBid(sender, message)
    ...
    -- Если ставка уже есть (highestBid существует), то новая должна быть строго больше.
    -- Если ставок еще нет, то новая должна быть больше или равна стартовой (hiBid).
    if highestBid and not (dkp > hiBid) then
        SOTA_whisper(sender, string.format("Ваша ставка должна быть больше текущей (%d DKP).", hiBid));
        return;
    elseif not highestBid and (dkp < hiBid) then
        SOTA_whisper(sender, string.format("Минимальная первая ставка: %d DKP.", hiBid));
        return;
    end;
    ...
    if not userWentAllIn then
        ...
        if dkp < minimumBid then
            SOTA_whisper(sender,
                string.format("Вам нужно сделать ставку не менее %s DKP - ставка не засчитана.", minimumBid));
            return;
        end
    end
...
function SOTA_UpdateBidElements()
    ...
    for n = 1, MAX_BIDS, 1 do
        ...
        getglobal(frame:GetName() .. "Bid"):SetTextColor((bidcolor[1] / 255), (bidcolor[2] / 255), (bidcolor[3] / 255),
            255);
